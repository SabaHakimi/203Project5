...
public final class Fairy {
    ...
    private boolean moveToFull(WorldModel world, Fairy target, EventScheduler scheduler) {
    ...
    private boolean moveToNotFull(WorldModel world, Fairy target, EventScheduler scheduler) {
        if (adjacent(this.position, target.position)) {
    ...
    private boolean moveToFairy(WorldModel world, Fairy target, EventScheduler scheduler) {
        if (adjacent(this.position, target.position)) {
    ...
    private boolean transformSapling(WorldModel world, EventScheduler scheduler, ImageStore imageStore) {
        if (this.health <= 0) {
            Fairy stump = Factory.createStump(EntityParsing.getStumpKey() + "_" + this.id, this.position, imageStore.getImageList(EntityParsing.getStumpKey()));
            ...
            Fairy tree = Factory.createTree(EntityParsing.getTreeKey() + "_" + this.id, this.position, getNumFromRange(TREE_ACTION_MAX, TREE_ACTION_MIN), getNumFromRange(TREE_ANIMATION_MAX, TREE_ANIMATION_MIN), getIntFromRange(TREE_HEALTH_MAX, TREE_HEALTH_MIN), imageStore.getImageList(EntityParsing.getTreeKey()));
    ...
    private void transformFull(WorldModel world, EventScheduler scheduler, ImageStore imageStore) {
        Fairy dude = Factory.createDudeNotFull(this.id, this.position, this.actionPeriod, this.getAnimationPeriod(), this.resourceLimit, this.images);
    ...
    private boolean transformNotFull(WorldModel world, EventScheduler scheduler, ImageStore imageStore) {
        if (this.resourceCount >= this.resourceLimit) {
            Fairy dude = Factory.createDudeFull(this.id, this.position, this.actionPeriod, this.getAnimationPeriod(), this.resourceLimit, this.images);
    ...
    public void executeDudeFullActivity(WorldModel world, ImageStore imageStore, EventScheduler scheduler) {
        Optional<Fairy> fullTarget = world.findNearest(this.position, new ArrayList<>(List.of(EntityKind.HOUSE)));
    ...
    public void executeDudeNotFullActivity(WorldModel world, ImageStore imageStore, EventScheduler scheduler) {
        Optional<Fairy> target = world.findNearest(this.position, new ArrayList<>(Arrays.asList(EntityKind.TREE, EntityKind.SAPLING)));
    ...
    public void executeFairyActivity(WorldModel world, ImageStore imageStore, EventScheduler scheduler) {
        Optional<Fairy> fairyTarget = world.findNearest(this.position, new ArrayList<>(List.of(EntityKind.STUMP)));
        ...
        if (fairyTarget.isPresent()) {
            ...
            if (this.moveToFairy(world, fairyTarget.get(), scheduler)) {
                ...
                Fairy sapling = Factory.createSapling(EntityParsing.getSaplingKey() + "_" + fairyTarget.get().id, tgtPos, imageStore.getImageList(EntityParsing.getSaplingKey()));
