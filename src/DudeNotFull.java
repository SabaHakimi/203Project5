 ...
 */
public final class DudeNotFull {
    ...
    private boolean moveToFull(WorldModel world, DudeNotFull target, EventScheduler scheduler) {
    ...
    private boolean moveToNotFull(WorldModel world, DudeNotFull target, EventScheduler scheduler) {
        if (adjacent(this.position, target.position)) {
    ...
    private boolean moveToFairy(WorldModel world, DudeNotFull target, EventScheduler scheduler) {
        if (adjacent(this.position, target.position)) {
    ...
    private boolean transformSapling(WorldModel world, EventScheduler scheduler, ImageStore imageStore) {
        if (this.health <= 0) {
            DudeNotFull stump = Factory.createStump(EntityParsing.getStumpKey() + "_" + this.id, this.position, imageStore.getImageList(EntityParsing.getStumpKey()));
            ...
            DudeNotFull tree = Factory.createTree(EntityParsing.getTreeKey() + "_" + this.id, this.position, getNumFromRange(TREE_ACTION_MAX, TREE_ACTION_MIN), getNumFromRange(TREE_ANIMATION_MAX, TREE_ANIMATION_MIN), getIntFromRange(TREE_HEALTH_MAX, TREE_HEALTH_MIN), imageStore.getImageList(EntityParsing.getTreeKey()));
    ...
    private void transformFull(WorldModel world, EventScheduler scheduler, ImageStore imageStore) {
        DudeNotFull dude = Factory.createDudeNotFull(this.id, this.position, this.actionPeriod, this.getAnimationPeriod(), this.resourceLimit, this.images);
    ...
    private boolean transformNotFull(WorldModel world, EventScheduler scheduler, ImageStore imageStore) {
        if (this.resourceCount >= this.resourceLimit) {
            DudeNotFull dude = Factory.createDudeFull(this.id, this.position, this.actionPeriod, this.getAnimationPeriod(), this.resourceLimit, this.images);
    ...
    public void executeDudeFullActivity(WorldModel world, ImageStore imageStore, EventScheduler scheduler) {
        Optional<DudeNotFull> fullTarget = world.findNearest(this.position, new ArrayList<>(List.of(EntityKind.HOUSE)));
    ...
    public void executeDudeNotFullActivity(WorldModel world, ImageStore imageStore, EventScheduler scheduler) {
        Optional<DudeNotFull> target = world.findNearest(this.position, new ArrayList<>(Arrays.asList(EntityKind.TREE, EntityKind.SAPLING)));
    ...
    public void executeFairyActivity(WorldModel world, ImageStore imageStore, EventScheduler scheduler) {
        Optional<DudeNotFull> fairyTarget = world.findNearest(this.position, new ArrayList<>(List.of(EntityKind.STUMP)));
        ...
        if (fairyTarget.isPresent()) {
            ...
            if (this.moveToFairy(world, fairyTarget.get(), scheduler)) {
                ...
                DudeNotFull sapling = Factory.createSapling(EntityParsing.getSaplingKey() + "_" + fairyTarget.get().id, tgtPos, imageStore.getImageList(EntityParsing.getSaplingKey()));
